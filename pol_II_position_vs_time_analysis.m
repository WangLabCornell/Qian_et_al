...
indirect_torque_raw = tracedata.torque;
dt = x_raw(2)-x_raw(1);
...
%I_start = find(indirect_torque_raw>19,1,"first");
I_start = 1;
if(I_end <= I_start)
    I_end = I_start+1;
end
...
indirect_torque = indirect_torque_raw(I_start:I_end);
...
plot(x_raw, indirect_torque_raw, 'LineWidth',2);
ylim([0,22]);
...
overall_torque = mean(indirect_torque);
